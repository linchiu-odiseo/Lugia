...
describe('Marcacion', () => {
  ...
  describe('construcción válida', () => {
    it('expone los tres campos cuando args son válidos', () => {
      const m = new Marcacion('exam-001', 5, validAlternativa);
      expect(m.examId).toBe('exam-001');
    ...
    it('trimea examId', () => {
      const m = new Marcacion('  exam-001  ', 1, validAlternativa);
      expect(m.examId).toBe('exam-001');
    });

    it('admite alternativa desmarcada (value === null)', () => {
      const m = new Marcacion('exam-001', 1, validUnmarked);
  ...
  describe('invariantes — examId', () => {
    it('rechaza examId vacío con InvalidMarcacionError', () => {
      expect(() => new Marcacion('', 1, validAlternativa)).toThrow(InvalidMarcacionError);
    });

    it('rechaza examId solo whitespace con InvalidMarcacionError', () => {
      expect(() => new Marcacion('   ', 1, validAlternativa)).toThrow(InvalidMarcacionError);
    });

    it('rechaza examId null-equivalent sin crashear', () => {
  ...
  describe('invariantes — pregunta', () => {
    it('rechaza pregunta = 0 con InvalidMarcacionError', () => {
      expect(() => new Marcacion('exam-001', 0, validAlternativa)).toThrow(InvalidMarcacionError);
    });

    it('rechaza pregunta negativa con InvalidMarcacionError', () => {
      expect(() => new Marcacion('exam-001', -1, validAlternativa)).toThrow(InvalidMarcacionError);
    });

    it('rechaza pregunta no-entera (1.5) con InvalidMarcacionError', () => {
      expect(() => new Marcacion('exam-001', 1.5, validAlternativa)).toThrow(InvalidMarcacionError);
    });

    it('rechaza pregunta NaN con InvalidMarcacionError', () => {
      expect(() => new Marcacion('exam-001', Number.NaN, validAlternativa)).toThrow(
        InvalidMarcacionError,
      );
    });
    ...
    it('rechaza pregunta no-numérica con InvalidMarcacionError', () => {
      expect(
        () => new Marcacion('exam-001', 'cinco' as unknown as number, validAlternativa),
  ...
  describe('invariantes — alternativa', () => {
    it('rechaza alternativa que no es instancia de Alternativa (string crudo)', () => {
      expect(() => new Marcacion('exam-001', 1, 'C' as unknown as Alternativa)).toThrow(
        InvalidMarcacionError,
      );
    });

    it('rechaza alternativa null con InvalidMarcacionError', () => {
      expect(() => new Marcacion('exam-001', 1, null as unknown as Alternativa)).toThrow(
        InvalidMarcacionError,
      );
    });

    it('rechaza alternativa undefined con InvalidMarcacionError', () => {
      expect(() => new Marcacion('exam-001', 1, undefined as unknown as Alternativa)).toThrow(
        InvalidMarcacionError,
      );
    });
    ...
    it('rechaza objeto que parece Alternativa pero no es instancia (duck-typing)', () => {
      ...
      expect(() => new Marcacion('exam-001', 1, fakeAlt)).toThrow(InvalidMarcacionError);
    });
  });
});
